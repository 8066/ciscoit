...
function init(isHtml)
{
	...
	try {
		...
		var courseName ="";
		if (location.href.includes("/trunk/")){	  			
			courseName = urlArray[urlArray.length-8];			
		}	else{
			courseName = urlArray[urlArray.length-7];
		}
      	runtime =  JSON.parse(localStorage.getItem(courseName+"/display"));
    } catch (error) {
